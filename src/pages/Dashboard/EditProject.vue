...
const { data: project, isLoading: isFetching, error: fetchError } = useGetQuery<any>({
  url: () => `/projects/${projectSlug.value}`,
  isPrivate: true,
  keys: [projectSlug],
});
...
// Media upload and existing images state
const existingImages = ref<string[]>([]);
const imageFiles = ref<File[]>([]);
const imagePreviews = ref<string[]>([]);
const isDragging = ref(false);

// Watch project changes to pre-populate form
watch(
  project,
  (newVal) => {
    if (newVal) {
      ...
      timeline.value = newVal.timeline || '';
      liveUrl.value = newVal.liveUrl || '';
      githubFrontendUrl.value = newVal.githubFrontendUrl || '';
      githubBackendUrl.value = newVal.githubBackendUrl || '';
      featured.value = newVal.featured || false;
      overview.value = newVal.overview || '';
      challenges.value = newVal.challenges || '';
      solution.value = newVal.solution || '';
      tagsInput.value = Array.isArray(newVal.tags) ? newVal.tags.join(', ') : '';
      features.value = Array.isArray(newVal.features) ? [...newVal.features] : [];
      existingImages.value = Array.isArray(newVal.images) ? [...newVal.images] : [];
    }
  },
  { immediate: true }
);
...
const handleSubmit = async () => {
  if (existingImages.value.length === 0 && imageFiles.value.length === 0) {
  ...
  try {
    ...
    // Append remaining existing images as JSON string
    ...
    successMessage.value = 'Project updated successfully! Redirecting...';
    setTimeout(() => {
      router.push('/dashboard/projects');
    }, 1500);
  } catch (err: any) {
    console.error(err);
    errorMessage.value = err.message || 'Failed to update project. Please try again.';
  }
};
</script>
  ...
  <div class="space-y-6 max-w-6xl mx-auto pb-12">
    ...
    <div class="flex items-center gap-4">
      <router-link
        ...
        class="p-2 bg-secondary hover:bg-secondary/80 border border-border/10 rounded-lg text-foreground transition-colors inline-flex items-center justify-center cursor-pointer"
      ...
      <div>
        <h4>Edit Project</h4>
        <p class="text-sm text-foreground/50">Modify and update your project details.</p>
      </div>
    </div>

    <!-- Loading State -->
    <div v-if="isFetching" class="flex flex-col items-center justify-center py-20 space-y-4">
      <PhSpinner class="animate-spin text-primary" :size="40" />
      <p class="text-sm text-foreground/60">Fetching project details...</p>
    </div>

    <!-- Error State -->
    <div v-else-if="fetchError" class="p-6 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 max-w-lg mx-auto text-center space-y-4">
      <p class="text-sm font-semibold">Failed to load project details.</p>
      <p class="text-xs opacity-80">{{ fetchError.message || 'Network error occurred.' }}</p>
      <router-link
        to="/dashboard/projects"
        class="inline-block px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-lg transition-colors"
      >
        Back to Projects
      </router-link>
    </div>
    ...
    <div v-else>
      ...
      <transition name="fade">
        <div v-if="errorMessage" class="p-4 mb-6 rounded-lg bg-red-500/10 border border-red-500/20 text-red-500 text-sm">
          {{ errorMessage }}
        </div>
      </transition>
      <transition name="fade">
        <div v-if="successMessage" class="p-4 mb-6 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-sm">
          {{ successMessage }}
        </div>
      </transition>
      ...
      <form @submit.prevent="handleSubmit" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Details Card -->
          <div class="p-6 rounded-xl bg-background/50 border border-border/10 dark:border-white/5 backdrop-blur-md space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <textarea
                ...
                class="w-full px-3 py-2.5 rounded-lg border border-border/20 dark:border-white/5 bg-background/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all text-sm resize-none"
              ></textarea>
            </div>
          </div>
          ...
          <div class="p-6 rounded-xl bg-background/50 border border-border/10 dark:border-white/5 backdrop-blur-md space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <!-- Challenges -->
              <div class="space-y-1.5">
                <label class="text-xs font-semibold text-foreground/75 uppercase tracking-wider">Challenges Encountered *</label>
                <textarea
                  v-model="challenges"
                  required
                  rows="4"
              ...
              <!-- Solution -->
              <div class="space-y-1.5">
                <label class="text-xs font-semibold text-foreground/75 uppercase tracking-wider">Solution Implemented *</label>
                <textarea
                  v-model="solution"
                  required
                  rows="4"
          ...
          <!-- Features List -->
          <div class="p-6 rounded-xl bg-background/50 border border-border/10 dark:border-white/5 backdrop-blur-md space-y-4 @container">
            <h5>Key Features</h5>
            ...
            <div class="flex gap-2 @max-xs:flex-col">
              <input
                v-model="newFeature"
                type="text"
                placeholder="E.g., Multi-tenant support, Stripe payments"
        ...
        <div class="space-y-6">
          ...
          <div class="p-6 rounded-xl bg-background/50 border border-border/10 dark:border-white/5 backdrop-blur-md space-y-4">
            ...
            <div v-if="existingImages.length > 0" class="space-y-2">
              <p class="text-xs font-semibold text-foreground/75 uppercase tracking-wider">Current Images</p>
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  <img :src="url" alt="Existing" class="w-full h-full object-cover" />
                  <button
                    type="button"
            ...
            <hr class="border-border/10 dark:border-white/5 my-2" />

            <p class="text-xs font-semibold text-foreground/75 uppercase tracking-wider">Upload New Images</p>
            ...
            >
              ...
              <span class="px-4 py-2 bg-primary hover:bg-primary/95 text-white text-xs font-semibold rounded-lg shadow-sm shadow-primary/20 hover:shadow-md transition-all">
                Browse Files
              </span>
          ...
          <div class="p-6 rounded-xl bg-background/50 border border-border/10 dark:border-white/5 backdrop-blur-md space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                type="text"
                required
                placeholder="E.g., 2 Months (Jan - Feb)"
                class="w-full px-3 py-2.5 rounded-lg border border-border/20 dark:border-white/5 bg-background/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all text-sm"
              />
            </div>
            ...
            <!-- Tags -->
            <div class="space-y-1.5">
              <label class="text-xs font-semibold text-foreground/75 uppercase tracking-wider">Technologies / Tags *</label>
              <input
                v-model="tagsInput"
                type="text"
                required
                placeholder="Vue 3, TypeScript, Express, MongoDB"
                class="w-full px-3 py-2.5 rounded-lg border border-border/20 dark:border-white/5 bg-background/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all text-sm"
              />
              <p class="text-[10px] text-foreground/45 font-medium">Separate technologies using commas.</p>
            </div>
          </div>
            ...
            <div class="flex items-center justify-between pt-2">
              ...
              >
                <span
                  ...
                  :class="[featured ? 'transform translate-x-5' : '']"
                ></span>
              </button>
            </div>
          </div>
